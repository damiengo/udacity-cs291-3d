...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom )
{
    ...
    // Top sphere
    var sphT = new THREE.Mesh( sphGeom, material );
    sphT.matrixAutoUpdate = false;
    sphT.matrix.makeTranslation( top.x, top.y, top.z );
    ob.add(sphT);
    // Bottom sphere
    var sphB = new THREE.Mesh( sphGeom, material );
    sphB.matrixAutoUpdate = false;
    sphB.matrix.makeTranslation( bottom.x, bottom.y, bottom.z );
    ob.add(sphB);
    ob.add(cyl);
